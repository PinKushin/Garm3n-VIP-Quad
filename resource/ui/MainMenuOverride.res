...
//		"bgcolor_override"	"0 0 0 240"
	...
	"ButtonsBG"
	{
		...
		"ypos"			"r40"
		"zpos"			"-9"
		// 197 -> 218 to cover ReportPlayerButton, which was added at the right
		// end of this row. The strip starts at c-275 and the new button ends at
		// c-57, so 218 is exactly the span of the icons it sits behind.
		"wide"			"218"
		"tall"			"20"
		"fillcolor"		"0 0 0 175"
		"visible"		"1"
	...
	//-----------------------------------------------------------------------
	// Panels the client creates that this menu never accounted for.
	//
	// A panel the client builds but the .res never mentions keeps whatever
	// position its constructor gave it, which is usually the top-left corner.
	// Omitting one is therefore not "it stays hidden", it is "it lands
	// somewhere arbitrary" -- so each is either placed deliberately or
	// explicitly zeroed below.
	//-----------------------------------------------------------------------

	// In-game report flow, added long after this menu was last touched. Placed
	// at the right end of the existing icon row; ButtonsBG was widened from 197
	// to 218 to keep the strip behind it. Stock uses glyph_create here, which is
	// already CallVoteButton's icon on this HUD -- glyph_alert is unused and
	// reads as a report rather than as a second "create".
	"ReportPlayerButton"
	{
		"ControlName"	"EditablePanel"
	...
	// Zeroed, not omitted. This HUD supplies its own SteamWorkshopButton and
	// ReplayBrowserButton, and its own SettingsButton/TF2SettingsButton, so the
	// stock counterparts would be duplicates sitting at their default position.
	// The SDK settings buttons only exist in SDK builds; defining them costs
	// nothing and stops them appearing if one is ever run.
	"WorkshopButton"
	{
		"ControlName"	"CExImageButton"
	...
	// Legacy VR mode. The feature is gone from the game; these are zeroed rather
	// than left undefined for the same top-left-corner reason as above.
	"VRBGPanel"
	{
		"ControlName"	"EditablePanel"
		"fieldName"		"VRBGPanel"
	...
	// The five below are carried over from stock unchanged. Each is either
	// hidden until the client shows it (SafeMode after a crash,
	// CompetitiveAccessInfoPanel, StoreHasNewItemsImage) or is a plain status
	// readout (NoGCMessage/NoGCImage, shown while the item server is down).
	// Restyling a panel that cannot be triggered on demand would be changing
	// something nobody can look at, so they keep stock styling.
	"SafeMode"
	{
		"ControlName"	"EditablePanel"
		"fieldname"		"SafeMode"
